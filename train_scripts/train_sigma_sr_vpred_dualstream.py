...
ENABLE_LORA = False  # Mainline: disable LoRA; keep as optional ablation.
...
def configure_pixart_trainable_params(pixart: nn.Module, train_x_embedder: bool = True, train_stage: str = "C"):
    ...
    trainable_block_ids = list(TRAINABLE_BLOCKS_STAGE_A)
    selective_tuning = bool(ENABLE_SELECTIVE_TUNING)
    if train_stage == "B":
        trainable_block_ids = list(TRAINABLE_BLOCKS_STAGE_B)
    elif train_stage == "C":
        trainable_block_ids = list(TRAINABLE_BLOCKS_STAGE_C)

    enabled_block_ids = []
    if selective_tuning and len(trainable_block_ids) > 0 and hasattr(pixart, "blocks"):
        depth = len(pixart.blocks)
        for i in sorted(set(int(v) for v in trainable_block_ids)):
            if i < 0 or i >= depth:
                continue
            enabled_block_ids.append(i)
    ...
    print(
        ...
        f"block_window={enabled_block_ids}"
    )
