...
class PixArtSigmaSRDualStream(PixArtSigmaSR):
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, adapter_cond=None, force_drop_ids=None, **kwargs):
        ...
        if self.dualstream_enabled and x.shape[1] < 8:
            raise ValueError(
                f"PixArtSigmaSRDualStream expects 8-channel input [z_t(4), z_lr(4)] when dualstream is enabled, got {x.shape[1]} channels"
            )
        z_lr = x[:, 4:8] if x.shape[1] >= 8 else x[:, :4]
        ...
        for i, block in enumerate(self.blocks):
            ...
            if i in src_features and i in self.injection_index_map and i < self.injection_cutoff_layer:
                ...
                res_strength = 1.0 if stage_name == "struct" else (0.35 if stage_name == "trans" else 0.0)
